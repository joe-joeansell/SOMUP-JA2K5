...
# My notes:
Methods seem to be based around creating a system of 'cues', which cue the program to do certain things at certain timestamps, tracked in terms of seconds from when the program starts. Since I want to potentially have multiple songs and give a user control over the song, this will probably need to be adapted to work on a per song basis somehow...

May need extra context/constraints to extract better results from chatbots, also try different ones (such as Claude)